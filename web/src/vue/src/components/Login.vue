...
<div class="wrap">
    <div v-if="!this.$store.getters['token/getIsLogin']" class="google-btn" @click="sendGoogleUrl">
        <p class="btn-text"><b>GOOGLE  ACCOUNT  LOGIN</b></p>
    </div>
    ...
    <div v-else class="google-btn-login" @click="handleLogout">
        ...
        <p class="btn-text-login"><b>로그아웃</b></p>
    </div>
</div>
...
import googleLogo from '@/assets/logoex.png';
export default {
    name: 'Login',
    methods:{
        handleLogout(){
    ...
    setup(){
        ...
        const state = reactive({
            googleUrl: 'http://localhost:8080/oauth2/authorize/google?redirect_uri=http://localhost:8080/oauth2/redirect',
