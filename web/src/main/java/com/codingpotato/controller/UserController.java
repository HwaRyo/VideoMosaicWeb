...
public class UserController {
    ...
    public void upload(HttpServletResponse response) throws Exception {
        ...
        int port = 50001;
        String path1 = "D:\\Git\\VideoMosaicWeb\\web\\src\\main\\resources\\slice_test2.mp4";
        String path2 = "D:\\Git\\VideoMosaicWeb\\web\\src\\main\\resources\\slice_test2.mp4";
        ...
        dos.writeUTF("SeungWook0502@gmail.com");
...
//            os.writeUTF(path);               //파일의 이름을 서버에 전송합니다.
        ...
        while ((len = fin2.read(buffer)) > 0) {     //FileInputStream을 통해 파일에서 입력받은 데이터를 버퍼에 임시저장하고 그 길이를 측정합니다.
            data++;                        //데이터의 양을 측정합니다.
        }
        ...
        byteArr = String.valueOf(data).getBytes("UTF-8");
...
//            os.writeUTF(path);               //파일의 이름을 서버에 전송합니다.
        ...
        for (; data > 0; data--) {                   //데이터를 읽어올 횟수만큼 FileInputStream에서 파일의 내용을 읽어옵니다.
            len = fin2.read(buffer);        //FileInputStream을 통해 파일에서 입력받은 데이터를 버퍼에 임시저장하고 그 길이를 측정합니다.
            os.write(buffer, 0, len);       //서버에게 파일의 정보(1kbyte만큼보내고, 그 길이를 보냅니다.
        }
