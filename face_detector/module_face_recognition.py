...
class module_face_recognition:
    ...
    def __init__(self, model_dir):  # base_idr -> 모델이 저장되어 있는 디렉토리
        ...
        self.model.read(os.path.join(base_dir, 'custom.xml'))

    def information(self):
        # information creating
        dirs = [d for d in glob.glob(self.base_dir + "/*") if os.path.isdir(d)]
        names = dict([])
        for dir in dirs:
